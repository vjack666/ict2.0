...
- MT5 Python `time` values are Unix epochs in UTC. A broker wall-clock display such as UTC+3 must not be subtracted. `MT5Adapter` rejects any nonzero `server_utc_offset_seconds`.
...
5. Verify no broker-wall-clock offset is passed to the adapter. Validate the UTC epoch directly against the host clock.
...
- A nonzero `server_utc_offset_seconds` is rejected. If an epoch is stale or future, fail closed and investigate the terminal/host clock; do not apply a broker display offset.
