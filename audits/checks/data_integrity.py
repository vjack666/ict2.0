"""Deterministic OHLC data-integrity audit."""
from __future__ import annotations

from typing import Any, Iterable

from audits.contracts.gate import Finding, GateStatus, AuditResult
...
def audit_ohlc(rows: Iterable[dict[str, Any]], audit_id: str = "A0_DATA_INTEGRITY") -> AuditResult:
    ...
    for idx, row in enumerate(rows):
        ...
        try:
            ...
            if not all(map(lambda v: v == v and abs(v) != float("inf"), (o, h, l, c))):
    ...
    return AuditResult(audit_id, status, len(rows), accepted, len(rows) - accepted, tuple(findings), {"invalid_rate": (len(rows)-accepted)/len(rows) if rows else 0.0})
