#!/usr/bin/env python3
"""MICRO-AGENTE B3 — INCREMENTALIDAD + sesgo H4 (experimento B3).

H_B3: 'Anadir filtro de sesgo H4 (entra solo si sesgo H4 alinea direccion con
...
Diseno estricto (contrato B1/B2/B3):
- MISMO motor run_sequential(structure_mode="lite") que B1/B2; MISMO commit de
  motor (daef67cf; engine/ es byte-identico entre daef67cf y HEAD).
- MISMA construccion de trade que B1/B2 (anchor = nodo STRUCTURE (ch.nodes[3]),
  mecha de sweep REAL (low[sweep_bar] si long / high[sweep_bar] si short), SL
  estructural de 2 terminos min(mecha sweep, swing roto)-buffer, TP
  measured_projection, horizonte 200, tie_policy=pessimistic). NO se cambia
  SL/TP.
- UNICO cambio permitido: FILTRO de entrada por sesgo H4. Se usa la columna
  'trend' de engine.market_features.build_features(H4) (BULLISH/BEARISH/RANGING),
  verificada PIT (FULL==PREFIX, 0 violaciones en smoke de este repo). El filtro
  mantiene un anchor solo si (dir==1 y H4 BULLISH) o (dir==-1 y H4 BEARISH);
  RANGING y contratendencia se descartan. B3 es un subconjunto filtrado de B1.
...
- Delta vs B1 con IC: bootstrap POR PARES agrupado por chain_id (B3* subconjunto
  de B1* en cada remuestreo) -> CI95 de Delta_expectancy y Delta_win_rate que
  respeta la dependencia subconjunto/superconjunto.

Salida: reports/audits/EXP_B3_raw.json + reports/audits/EXP_B3_audit.json
"""
...
# ---- Protocolo fijo (identico a exp_agentA_runner / B1 / B2) ----
...
DEPTH_MIN = 4
TF = "H1"
CODE_COMMIT = "daef67cf212c4432c6e5e3a2b7c6cd404982059b"  # mismo motor que B1/B2
...
RAW_PATH = REPORT_DIR / "EXP_B3_raw.json"
AUDIT_PATH = REPORT_DIR / "EXP_B3_audit.json"
...
def build_trade(direction, entry_bar, close, high, low, swing_lows, swing_highs,
    ...
    """IDENTICO a exp_agentA_runner.build_trade (B1/B2)."""
...
def compute_metrics(trades, cluster_key):
    """IDENTICO a exp_agentA_runner.compute_metrics (B1/B2)."""
...
def build_h4_trend_timeline(h4_df: pd.DataFrame):
    """Construye el timeline de sesgo H4 (columna 'trend' de build_features).

    PIT-safe: build_features en el frame completo; la columna 'trend' se
    verifica FULL==PREFIX (muestreo 5 barras). Devuelve
    (h4_times_sorted, h4_trend_list, viol) para mapear por bisect cada barra H1
    a la ultima vela H4 cerrada <= t.
    """
    ann = build_features(h4_df)
    if "trend" not in ann.columns:
        raise RuntimeError("build_features(H4) no expone columna 'trend'")
    ...
    viol = 0
    for i in idxs:
        pref = build_features(h4_df.iloc[: i + 1].copy())
...
def h4_trend_at(h1_time: pd.Timestamp, h4_times, h4_trend) -> str:
    """Sesgo H4 cerrado mas reciente <= h1_time (PIT-safe lookup)."""
    j = bisect_right(h4_times, h1_time) - 1
    if j < 0:
        return "RANGING"
    return str(h4_trend.iloc[j])


def passes_h4_filter(direction: int, h4_trend: str) -> bool:
    """Filtro B3: long<->BULLISH, short<->BEARISH; RANGING/counter => drop."""
    if direction == 1:
        return h4_trend == "BULLISH"
    if direction == -1:
        return h4_trend == "BEARISH"
    return False
...
def paired_delta_bootstrap(trades_all: list[dict], passes_flag, seed=42, n=2000):
    ...
    trades cerrados del resample; B3* = los que pasan el filtro H4. Delta* =
    media(B3*) - media(B1*) y wr(B3*) - wr(B1*). El CI respeta que B3 es
        ...
        for idx in pick:
            ...
            for t in grp:
                ...
                if r > 0:
                    b1_w += 1
                if passes_flag(t):
                    b3_rs.append(r); b3_n += 1
                    if r > 0:
                        b3_w += 1
        if not b1_rs:
            continue
        mean_b1 = float(np.mean(b1_rs))
        mean_b3 = float(np.mean(b3_rs)) if b3_rs else 0.0
        d_r.append(mean_b3 - mean_b1)
        wr_b1 = b1_w / b1_n if b1_n else 0.0
        wr_b3 = b3_w / b3_n if b3_n else 0.0
        d_wr.append(wr_b3 - wr_b1)
    if not d_r:
        return {"delta_expectancy_ci95": None, "delta_win_rate_ci95": None, "n_boot": 0}
    return {
...
def main() -> None:
    ...
    # --- H4 bias timeline (PIT-safe) ---
    h4_times, h4_trend, pit_viol = build_h4_trend_timeline(h4_df)
    if pit_viol > 0:
        raise RuntimeError(f"PIT debt en trend H4: {pit_viol} violaciones FULL vs PREFIX -> BLOCKED")
    print(f"H4 timeline OK; PIT violations={pit_viol}; H4 range "
          f"{h4_times.iloc[0]} .. {h4_times.iloc[-1]}", flush=True)

    # --- run_sequential (mismo motor B1/B2) ---
    ...
    for ch in candidates:
        ...
        if key in seen or bar_i < WARMUP_BARS:
            continue
        seen.add(key)
        # REAL sweep wick (misma logica que B1/B2 / exp_agentA_runner)
        ...
        # H4 bias al tiempo del anchor (PIT-safe: ultima vela H4 cerrada <= t)
        h4b = h4_trend_at(h1_dt.iloc[bar_i], h4_times, h4_trend)
        treatment.append({
    ...
    # --- FILTRO H4: mantener solo anchors alineados con sesgo H4 ---
    kept, dropped = [], []
    n_ranging = n_counter = 0
    for t in treatment:
        if passes_h4_filter(int(t["direction"]), t["h4_trend"]):
            kept.append(t)
        else:
            dropped.append(t)
            if t["h4_trend"] == "RANGING":
                n_ranging += 1
            else:
                n_counter += 1
    ...
    # --- METRICAS B3 (tratamiento filtrado) ---
    m_b3 = compute_metrics(kept, "chain_id")

    # --- DELTA vs B1 (comparador canonico en disco) ---
    b1_wr = float(b1_ctrl["treatment"]["win_rate"])
    delta_exp = round(m_b3["mean_r"] - b1_mean_r, 4) if m_b3.get("mean_r") is not None else None
    delta_wr = round(m_b3["win_rate"] - b1_wr, 4) if m_b3.get("win_rate") is not None else None
    # delta CI (paired bootstrap, B3 subset vs B1 superset)
    boot = paired_delta_bootstrap(
        treatment,
        lambda t: passes_h4_filter(int(t["direction"]), t["h4_trend"]),
    ...
    raw = {
        "schema_version": "1.0",
        "experiment": "EXP_B3",
        "role": "INCREMENTAL_H4_BIAS_FILTER (subconjunto filtrado de B1)",
        "hypothesis": (
            "H_B3: Anadir filtro de sesgo H4 (entra solo si sesgo H4 alinea direccion con "
            "la estructura) incrementa la expectancy del baseline LTF (B1) en R por "
            "operacion, bajo protocolo identico. Metrica: expectancy (mean_R). Gate: "
        ...
        "dataset": {
            ...
            "note": "Dataset canonico H1 2019-2024 (mismo que B1/B2).",
        },
        "htf_input": {
        ...
        "config": {
            ...
            "htf_context": True,
            "htf_filter": "keep iff (dir==1 and H4 BULLISH) or (dir==-1 and H4 BEARISH); RANGING/counter dropped",
        },
        "fecha": datetime.now(timezone.utc).isoformat(),
        "treatment": {
            "n_closed": m_b3.get("n_closed"),
            "n_trades": m_b3.get("n_trades"),
            "n_open": m_b3.get("n_open"),
            "mean_R": m_b3.get("mean_r"),
            "median_R": m_b3.get("median_r"),
            "win_rate": m_b3.get("win_rate"),
            "profit_factor": m_b3.get("profit_factor"),
            "expectancy": m_b3.get("expectancy"),
            "drawdown": m_b3.get("drawdown"),
            "wilson_95": m_b3.get("win_rate_wilson95"),
            "bootstrap_ci_95": (m_b3.get("bootstrap") or {}).get("mean_r_ci"),
        },
        "baseline": None,
        "baseline_note": "El comparador de B3 es B1 (control LTF en disco). No se genera baseline propio.",
        "filter": {
            "method": "H4 trend alignment via engine.market_features.build_features(H4)['trend']",
        ...
        "protocol": {
            "leakage_check": (
                ...
                "(PIT-estable DENTRO del rango). El sesgo H4 se obtiene de build_features(H4)"
                "['trend'], columna PIT-verificada (FULL==PREFIX, 0 violaciones): se lee el "
                "valor en la ultima vela H4 cerrada <= t del anchor H1. Sin leakage futuro."
            ),
            ...
            "data_integrity": {
                ...
                "range_start": RANGE_START, "range_end": RANGE_END, "bars": n_bars,
                "dataset_hash": ds_hash, "is_canonical": True, "origin": "csv",
                "htf_source": H4_REL, "htf_dataset_hash": h4_hash,
                "note": "Dataset canonico H1 verificado por SHA256; mismo motor/commit que B1/B2.",
            },
        },
        "elapsed_s": elapsed,
    }
    ...
    # ===== AUDIT =====
    n_closed = m_b3.get("n_closed") or 0
    ci = (m_b3.get("bootstrap") or {}).get("mean_r_ci")
    ci_lower_gt_0 = bool(ci and len(ci) == 2 and ci[0] > 0)
    if n_closed < MIN_N_GATE:
        verdict = "BLOCKED"
    elif (m_b3.get("mean_r") or 0) > 0 and ci_lower_gt_0:
        verdict = "PASS"
    else:
        verdict = "FAIL"
    ...
    audit = {
        ...
        "gate": {
            ...
            "expectancy_gt_0": bool((m_b3.get("mean_r") or 0) > 0),
    ...
    print(json.dumps({
        "experiment": "EXP_B3",
        "verdict": verdict,
        "treatment": {k: m_b3.get(k) for k in ("n_closed", "n_trades", "n_open",
